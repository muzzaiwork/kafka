...
### 3. 재시도 동작 원리의 이해 (중요)
스프링 카프카의 `@RetryableTopic`은 단순히 애플리케이션 내부에서 루프를 돌며 재처리하는 방식이 아니다.
1. **메시지 재전송**: 처리에 실패하면 해당 메시지를 내부적으로 생성된 **별도의 재시도용 토픽**으로 다시 보낸다.
2. **재소비**: 컨슈머는 일정 시간이 지난 후 재시도용 토픽에 쌓인 메시지를 **다시 읽어와서(Consume)** 처리를 시도한다.
3. **상태 관리**: 이는 애플리케이션이 종료되었다가 다시 켜져도 재시도 정보가 카프카에 남아있어 안정적으로 재시도를 이어갈 수 있음을 의미한다.
