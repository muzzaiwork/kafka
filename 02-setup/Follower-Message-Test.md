...
sequenceDiagram
    ...
    participant N1 as 노드 1 (리더)

    P->>N2: 메시지 전송 시도 (19092)
    Note over N2: "나 리더 아니야, 1번 노드가 리더야"
    N2-->>P: 리더 정보 전달 (Metadata)
    P->>N1: 실제 메시지 전송 (9092)
    Note over N1: 메시지 저장 및 팔로워에게 복제
```
...
서버들이 정상적으로 메시지를 복제하고 있음을 확인했다면, 이제 실제 장애 상황을 가정하여 리더 노드가 다운되었을 때 시스템이 어떻게 대응하는지 실습해본다.

- [[실습] 리더 장애 및 고가용성 테스트](./Leader-Failure-Test.md)
