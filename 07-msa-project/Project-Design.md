...
5. **대규모 데이터 처리 흐름 (10만 건의 메시지 시나리오)**:
   - **1단계: 메시지 발생 (10만 건)**: 짧은 시간 내에 10만 건의 회원가입 요청이 들어오면, User Service는 10만 개의 이벤트를 생성한다.
   - **2단계: 파티션 분산 저장 (Sticky/Round Robin)**: 프로듀서는 이 메시지들을 3개의 파티션(P0, P1, P2)으로 나누어 보낸다. 이때 효율을 위해 메시지를 묶어서 보낼 수 있으며(Batching), 특정 파티션에 효율적으로 몰아주는 **스티키(Sticky)** 정책이나 순차적으로 보내는 **라운드 로빈** 방식을 사용하여 노드들에 분산 저장한다.
   - **3단계: 데이터 복제 및 안전 보관**: 각 파티션의 리더는 메시지를 저장하고, 즉시 2개의 팔로워들에게 데이터를 복제하여 총 3개의 복제본(RF=3)을 유지한다. 이 과정은 매우 빨라 대량의 데이터도 안전하게 클러스터에 쌓인다.
   - **4단계: 병렬 소비 및 처리**: Email Service의 컨슈머 그룹(3대)은 각자 맡은 파티션(P0, P1, P2)에서 메시지를 1/3씩 나누어 동시에 가져온다. 
   - **5단계: 지연 해소**: 초반에는 메시지가 쌓여 **컨슈머 랙(Lag)**이 10만까지 치솟을 수 있지만, 컨슈머들이 쉬지 않고 병렬로 처리하면서 점차 랙이 줄어들고 결국 모든 메시지가 이메일 발송까지 성공하게 된다.
